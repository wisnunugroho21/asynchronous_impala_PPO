...
class Memory(Dataset):
    ...
    def __getitem__(self, idx):
        return np.array(self.states[idx], dtype = np.float32), np.array(self.actions[idx], dtype = np.float32), np.array(self.action_probs[idx], dtype = np.float32), \
    ...
    def get_all(self):
        return self.states, self.actions, self.action_probs, self.rewards, self.dones, self.next_states
    
    def save_all(self, states, actions, action_probs, rewards, dones, next_states):
        self.states = states
    ...
    def save_list(self, states, actions, action_probs, rewards, dones, next_states):
        self.states += states
        self.actions += actions
        self.action_probs += action_probs
        self.rewards += rewards
    ...
    def save_eps(self, state, action, action_prob, reward, done, next_state):
...
class Learner():
    ...
    def save_all(self, states, actions, action_probs, rewards, dones, next_states):
        self.memory.save_all(states, actions, action_probs, rewards, dones, next_states)

    def save_list(self, states, actions, action_probs, rewards, dones, next_states):
        self.memory.save_list(states, actions, action_probs, rewards, dones, next_states)

    # Loss for PPO  
    def get_loss(self, action_prob, values, old_action_prob, old_values, next_values, actions, rewards, dones, worker_action_probs):
        # Don't use old value in backpropagation
        Old_values      = old_values.detach()

        # Finding the ratio (pi_theta / pi_theta__old):        
        logprobs        = self.distributions.logprob(action_prob, actions)
        Old_logprobs    = self.distributions.logprob(old_action_prob, actions).detach()
        Worker_logprobs = self.distributions.logprob(worker_action_probs, actions).detach()        
        ...
        Kl              = self.distributions.kl_divergence(old_action_prob, action_prob)
        ...
        dist_entropy    = self.distributions.entropy(action_prob).mean()
    ...
    def training_ppo(self, states, actions, worker_action_probs, rewards, dones, next_states):                 
        action_prob, values          = self.actor(states), self.critic(states)
        old_action_prob, old_values  = self.actor_old(states), self.critic_old(states)
        next_values                  = self.critic(next_states)

        loss = self.get_loss(action_prob, values, old_action_prob, old_values, next_values, actions, rewards, dones, worker_action_probs)
    ...
    def update_ppo(self):        
        ...
        for _ in range(self.PPO_epochs):       
            for states, actions, action_probs, rewards, dones, next_states in dataloader:
                self.training_ppo(states.float().to(device), actions.float().to(device), action_probs.float().to(device), \
...
class Agent:  
    ...
    def save_eps(self, state, action, action_prob, reward, done, next_state):
        self.memory.save_eps(state, action, action_prob, reward, done, next_state)
    ...
    def act(self, state):
        ...
        if self.is_training_mode:
            # Sample the action
            action  = self.distributions.sample(action_prob)
        else:
            action  = action_prob  
              
        return action.squeeze(0).numpy(), action_prob.squeeze(0).detach().numpy()
...
class Runner():
    ...
    def run_episode(self, i_episode, total_reward, eps_time):
        ...
        for _ in range(self.n_update):
            action, action_probs        = self.agent.act(self.states)
            action                      = int(action)
            ...
            if self.training_mode:
                self.agent.save_eps(self.states.tolist(), action, action_probs, reward, float(done), next_state.tolist())
